...
@PIPELINES.register_module()
class RandomUpDownFlip(RandomFlip):
    def __init__(self, flip_ratio=None):
        super().__init__(flip_ratio=flip_ratio, direction='vertical')
    ...
    def __call__(self, results):
        ...
        if results[key]:
            ...
            for key in results.get('seg_fields', []):
                # use copy() to make numpy stride positive
                results[key] = mmcv.imflip(results[key], direction=self.direction).copy()
        return results
